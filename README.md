...
(function(){
  ...
  function gerarTexto() {
    ...
    const nome = estacao.name.toUpperCase();
    ...
    const tipo = estacoesPrincipais.includes(nome) ? "ESTAÇÃO DE" : "PONTO DE PARADA DE";

    let texto = `*🚊 Acompanhamento ${prefix}*\n\n*🚉 ${tipo} ${nome}*\n\nData : ${data}\n`;
    if (chegada) texto += `Posicionado às: ${chegada}h\n`;
    if (partida) texto += `Partimos às: ${partida}h\n`;
    ...
    if (partida && estacao.dep) {
      let atraso = toMinutes(partida) - toMinutes(estacao.dep);
      if (atraso < -720) atraso += 1440;
      if (atraso < 0) atraso = 0;
      texto += `Tempo de Atraso: ${formatMinutes(atraso)}h\n`;
    } else if (ehDestino && chegada && estacao.arr) {
      let atraso = toMinutes(chegada) - toMinutes(estacao.arr);
      if (atraso < -720) atraso += 1440;
      if (atraso < 0) atraso = 0;
      texto += `Tempo de Atraso na Chegada Final: ${formatMinutes(atraso)}h\n`;
    }
    ...
    if (!ehDestino && partida && estacao.dep) {
      ...
      const chegadaPrevista = (toMinutes(partida) + (tempoPrevisto < 0 ? tempoPrevisto + 1440 : tempoPrevisto)) % 1440;
      const h = Math.floor(chegadaPrevista / 60);
